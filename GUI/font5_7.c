...
/* 5*7字体定义 */
code const uint8 FONT5x7ASCII[][8] = {
/* 空格 */  
  {
   ________,
   ________,
...
****************************************************************************/
uint8  GUI_PutChar(u8 x, u8 y, uint8 ch)
{  uint8   font_dat;
   uint8   i, j;
   TCOLOR  bakc;
...
****************************************************************************/
void  GUI_PutString(u8 x, u8 y, char *str)
{  while(1)
   {  if( (*str)=='\0' ) break;
...
* 说明：操作失败原因是指定地址超出有效范围。
****************************************************************************/
void  GUI_PutNoStr(u8 x, u8 y, char *str, uint8 no)
{  if(no==0) return ;
   for(; no>0; no--)
   {  if( (*str)=='\0' ) break;
      if( GUI_PutChar(x, y, *str++)==0 ) break;
...
/*************************************************
* 函数名称: void LCD12864_NUMI_WRITE(u8 x,u8 y,float num) //不能使用long int类型，否则无法显示
* 描述: 向lcd写数字函数
* 输入: x,y：需要显示的位置，
				num：需要显示的数字
* 其他说明: 此函数仍在测试阶段，可能会出现异常
*************************************************/
void GUI_PutNum(u8 x,u8 y,float num) //不能使用long int类型，否则无法显示
{
    unsigned char num_string[5];
    sprintf(num_string,"%5f",num);
    GUI_PutString(x,y,num_string);
}
